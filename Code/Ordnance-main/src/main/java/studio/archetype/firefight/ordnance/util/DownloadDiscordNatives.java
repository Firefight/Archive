package studio.archetype.firefight.ordnance.util;
...
public class DownloadDiscordNatives {
    public static File downloadDiscordLibrary() throws IOException {
        ...
        String zipPath = "lib/" + arch + "/" + name + suffix; // Path of Discord's library inside the ZIP
        ...
        while((entry = zin.getNextEntry()) != null) {
            if(entry.getName().equals(zipPath)) {
                ...
                Files.copy(zin, temp.toPath()); // Copy the file in the ZIP to our temporary file
                zin.close(); // We are done, so close the input stream
                return temp; // Return our temporary file
            }
        ...
        zin.close();
        return null; // We couldn't find the library inside the ZIP
    }
}
