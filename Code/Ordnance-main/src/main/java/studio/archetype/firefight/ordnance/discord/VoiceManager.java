package studio.archetype.firefight.ordnance.discord;
...
import studio.archetype.firefight.ordnance.util.DownloadDiscordNatives;
...
public class VoiceManager {
    ...
    public VoiceManager() {
        ...
        this.params.setClientID(932435955741233203L);
        this.core = new Core(params);
    }

    public void start() {
        runCallbacksFuture = executor.scheduleAtFixedRate(core::runCallbacks, 0, 16, TimeUnit.MILLISECONDS);

        LobbySearchQuery query = core.lobbyManager().getSearchQuery();
        query.filter("metadata.example", LobbySearchQuery.Comparison.EQUAL, LobbySearchQuery.Cast.STRING, "voice");
    ...
    private void startVoice(Result result, Lobby lobby) {
        ...
        executor.scheduleAtFixedRate(()->{
            for(long uid : core.lobbyManager().getMemberUserIds(lobby)) {
                if(uid == core.userManager().getCurrentUser().getUserId()) continue;
    ...
    public static void loadDiscord() {
        try {
            File discordLibrary = DownloadDiscordNatives.downloadDiscordLibrary();
            if (discordLibrary == null) {
                System.err.println("Error downloading Discord SDK.");
                System.exit(-1);
            }
            Core.init(discordLibrary); // Initialize the Core
