...
public class RadarDebugRenderer {
    ...
    public void render(MatrixStack matrices, float tickDelta, VertexConsumerProvider.Immediate vertexConsumers, Camera cam) {
        ...
        int maxHeight = MotionTrackerManager.INSTANCE.getSettings().getTotalHeight();
        int thresholdHeight = MotionTrackerManager.INSTANCE.getSettings().getHeightThreshold();
        int maxDistance = MotionTrackerManager.INSTANCE.getSettings().getTotalDistance();
        int innerRing = MotionTrackerManager.INSTANCE.getSettings().getInnerRingDistance();
        float reach = MotionTrackerManager.INSTANCE.getSettings().getAttackRange();
    ...
    private void drawLineStrip(MatrixStack stack, Vec3d pos, Vec3d color, VertexConsumer consumer) {
        consumer.vertex(stack.peek().getPositionMatrix(), (float)pos.getX(), (float) pos.getY(), (float) pos.getZ())
    ...
    private void drawLine(MatrixStack stack, Vec3d pos, Vec3d pos2, Vec3d color, VertexConsumer consumer) {
        consumer.vertex(stack.peek().getPositionMatrix(), (float)pos.getX(), (float) pos.getY(), (float) pos.getZ())
                .color((float)color.getX(), (float)color.getY(), (float)color.getZ(), 1.0F)
                .next();
        consumer.vertex(stack.peek().getPositionMatrix(), (float)pos2.getX(), (float) pos2.getY(), (float) pos2.getZ())
                .color((float)color.getX(), (float)color.getY(), (float)color.getZ(), 1.0F)
                .next();
    }
